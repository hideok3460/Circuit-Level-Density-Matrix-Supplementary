...
\date{Supplement to ``Circuit-Level Density-Matrix Channel-Equivalence Verification of a Five-Qubit Coherent EPR-State Transfer Network with a Two-Component GHZ Resource Family''}

\begin{document}
\maketitle

\section*{Purpose and relation to the main manuscript}
This supplementary note contains the extended controlled-$X$ derivation that was formerly included as Appendix B of the main manuscript.  It is separated from the article because the result is supporting notation rather than a central contribution of the channel-equivalence theorem.  The derivation fixes the computational-basis convention, gives a compact Dirac-expanded representation of $\CX_{i,j,q}$, embeds the operator explicitly in an arbitrary $q$-qubit tensor product, and checks the two- and three-qubit conventions used by the manuscript.  The notation is motivated by the Dirac extension of CNOT gates discussed by Yamanashi, Ozawa, and Okazaki~\cite{Yamanashi2023}.
...
\label{eq:SCX01}
\end{equation}
For the two-qubit input used in the main manuscript,
\begin{align}
\CX_{0,1,2}
\left(
\ket{y}_1\otimes
\left[\alpha\ket{0}_0+(-1)^x\beta\ket{1}_0\right]
\right)
&=
\alpha\ket{y,0}+(-1)^x\beta\ket{\bar y,1}.
\label{eq:SBxy}
\end{align}
Thus the controlled-$X$ convention reproduces the encoded state definition used in the manuscript.
